...
namespace DVDManagement.Data
{
        ...
        private static async Task<Admin> CreateDefaultUser(UserManager<Admin> um, ILogger<DbInitializer> logger, string email, string password)
        {
            ...
            var ir = await um.CreateAsync(user, password);
            if (ir.Succeeded)
            {
